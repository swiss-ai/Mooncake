...
int CxiContext::construct(size_t num_cq_list, size_t num_comp_channels,
                          uint8_t port, int gid_index, size_t max_cqe,
                          int max_endpoints) {
    endpoint_store_ = std::make_shared<CxiEndpointStore>();
...
int CxiContext::registerMemoryRegionInternal(void *addr, size_t length,
    ...
    // For CXI, we need local read/write and remote read/write
    ...
    mrMeta.associated = false;

    LOG(INFO) << "registered memory region with key: " << mrMeta.key << "\n";
...
std::shared_ptr<CxiEndpoint> CxiContext::endpoint(
    ...
    auto new_endpoint = std::make_shared<CxiEndpoint>(*this);
    if (!cq_list_.empty() && cq_list_[0]) {
        int ret = new_endpoint->construct(cq_list_[0]->cq);
        if (ret != 0) {
            LOG(ERROR) << "Failed to construct CXI endpoint";
            return nullptr;
        }
    }
...
int CxiContext::pollCq(int max_entries, int cq_index) {
    ...
    struct fi_cq_data_entry entries[256];
    int to_poll = std::min(max_entries, 256);
