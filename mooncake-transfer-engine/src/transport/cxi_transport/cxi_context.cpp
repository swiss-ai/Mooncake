...
// CxiContext implementation
CxiContext::CxiContext(CxiTransport &engine, const std::string &device_name)
    : engine_(engine),
...
CxiContext::~CxiContext() {
    if (fabric_) {
        deconstruct();
    }
}

int CxiContext::construct(size_t num_cq_list, size_t max_cqe,
                          int max_endpoints) {

    // Setup hints for CXI provider
    hints_ = fi_allocinfo();
    if (!hints_) {
    ...
    LOG(INFO) << "CXI device (libfabric): " << device_name_
...
int CxiContext::buildSharedEndpoint(size_t max_wr, size_t max_inline) {
    (void)max_inline;
    ...
    LOG(INFO) << "device " << deviceName() << " ptr address: " << &shared_ep_->fid;
    local_ep_addr_.resize(addr_len);
    return 0;
}

int CxiContext::deconstruct() {
...
int CxiContext::registerMemoryRegionInternal(void *addr, size_t length,
                                             int access,
    ...
    if (length > (size_t)globalConfig().max_mr_size) {
        ...
        LOG(ERROR) << "shared ep must be built before registering memory regions!";
    ...
    ret = fi_mr_enable(mrMeta.mr);
    if (ret) {
        LOG(ERROR) << "fi_mr_enable failed for " << addr << ": " << fi_strerror(-ret);
        return ERR_CONTEXT;
    }
...
int CxiContext::registerMemoryRegion(void *addr, size_t length, int access) {
    CxiMemoryRegionMeta mrMeta;
...
int CxiContext::unregisterMemoryRegion(void *addr) {
    RWSpinlock::WriteGuard guard(mr_lock_);
...
int CxiContext::preTouchMemory(void *addr, size_t length) {
...
uint64_t CxiContext::rkey(void *addr) {
...
uint64_t CxiContext::lkey(void *addr) {
    RWSpinlock::ReadGuard guard(mr_lock_);
    auto it = mr_map_.upper_bound((uint64_t)addr);
    if (it != mr_map_.begin()) {
        --it;
...
void *CxiContext::mrDesc(void *addr) {
    RWSpinlock::ReadGuard guard(mr_lock_);
...
std::shared_ptr<CxiEndpoint> CxiContext::endpoint(
    ...
    auto new_ep = std::make_shared<CxiEndpoint>(*this);
...
std::shared_ptr<CxiEndpoint> CxiContext::peekEndpoint(
    const std::string& peer_nic_path) {
    RWSpinlock::ReadGuard guard(peer_map_lock_);
...
int CxiContext::deleteEndpoint(const std::string &peer_nic_path) {
    std::shared_ptr<CxiEndpoint> ep;
    {
        RWSpinlock::WriteGuard guard(peer_map_lock_);
        auto it = peer_map_.find(peer_nic_path);
...
int CxiContext::disconnectAllEndpoints() {
    RWSpinlock::WriteGuard guard(peer_map_lock_);
    for (auto& entry : peer_map_) {
...
size_t CxiContext::getTotalQPNumber() const {
    RWSpinlock::ReadGuard guard(peer_map_lock_);
    return peer_map_.size();
}

std::string CxiContext::nicPath() const {
    return engine_.local_server_name() + "@" + device_name_;
}

std::string CxiContext::localAddr() const {
...
std::string CxiContext::localEpAddr() const {
...
int CxiContext::insertPeerAddr(const std::string& peer_hex_addr,
                               fi_addr_t& out) {
...
int CxiContext::insertPeerAddrBytes(const uint8_t* addr, size_t len,
                                    fi_addr_t& out) {
...
void CxiContext::removePeerAddr(fi_addr_t fi_addr) {
...
int CxiContext::pollCq(int max_entries, int cq_index) {
    ...
    if (ret > 0) {
        ...
        for (ssize_t i = 0; i < ret; i++) {
            CxiOpContext* op_ctx =
                reinterpret_cast<CxiOpContext*>(entries[i].op_context);
