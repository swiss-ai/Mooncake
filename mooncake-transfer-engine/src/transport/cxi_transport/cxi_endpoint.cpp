...
CxiEndpoint::CxiEndpoint(CxiContext &context)
    : context_(context),
...
int CxiEndpoint::construct(struct fid_cq *cq, size_t num_qp_list,
                           size_t max_sge, size_t max_wr, size_t max_inline) {
    if (status_.load(std::memory_order_relaxed) != INITIALIZING) {
        LOG(ERROR) << "CXI Endpoint has already been constructed";
        return ERR_ENDPOINT;
    }
    std::cout << "creating endpoint for context " << context_.deviceName() << "\n";
    ...
    std::cout << "endpoint with local address: " << local_addr_.size() << "\n";
    local_addr_.resize(local_addr_len_);
...
int CxiEndpoint::destroyQP() { return deconstruct(); }

void CxiEndpoint::setPeerNicPath(const std::string &peer_nic_path) {
    RWSpinlock::WriteGuard guard(lock_);
    if (connected()) {
        LOG(WARNING) << "Previous CXI connection will be discarded";
        disconnectUnlocked();
    }
    peer_nic_path_ = peer_nic_path;
}

std::string CxiEndpoint::getLocalAddr() const {
...
int CxiEndpoint::insertPeerAddr(const std::string &peer_addr) {
...
int CxiEndpoint::setupConnectionsByActive() {
    RWSpinlock::WriteGuard guard(lock_);
    if (connected()) {
        LOG(INFO) << "CXI Connection has been established";
        return 0;
    }
    ...
    // Store our CXI endpoint address in cxi_addr field (hex encoded)
    local_desc.cxi_addr = getLocalAddr();
    ...
    if (peer_server_name.empty() || peer_nic_name.empty()) {
        LOG(ERROR) << "Parse peer CXI nic path failed: " << peer_nic_path_;
    ...
    rc = insertPeerAddr(peer_desc.cxi_addr);
    if (rc != 0) {
        return rc;
    }
...
int CxiEndpoint::setupConnectionsByPassive(const HandShakeDesc &peer_desc,
                                           HandShakeDesc &local_desc) {
    RWSpinlock::WriteGuard guard(lock_);
    if (connected()) {
        LOG(WARNING) << "Re-establish CXI connection: " << toString();
        disconnectUnlocked();
    }

    if (peer_desc.peer_nic_path != context_.nicPath() ||
        peer_desc.local_nic_path != peer_nic_path_) {
        local_desc.reply_msg = "CXI nic path inconsistency";
        LOG(ERROR) << "Invalid argument: peer CXI nic path inconsistency"
    ...
    if (peer_desc.cxi_addr.empty()) {
        local_desc.reply_msg = "No CXI address provided";
        LOG(ERROR) << "Peer did not provide CXI address";
        return ERR_REJECT_HANDSHAKE;
    }

    int ret = insertPeerAddr(peer_desc.cxi_addr);
    if (ret != 0) {
        local_desc.reply_msg = "Failed to insert peer address";
        return ret;
    }

    // Provide our address to peer (using cxi_addr field, not reply_msg)
    local_desc.local_nic_path = context_.nicPath();
    local_desc.peer_nic_path = peer_nic_path_;
    local_desc.cxi_addr = getLocalAddr();
    ...
    status_.store(CONNECTED, std::memory_order_release);
    VLOG(1) << "CXI connection established (passive): " << toString();
    return 0;
}

void CxiEndpoint::disconnect() {
    RWSpinlock::WriteGuard guard(lock_);
    disconnectUnlocked();
}

void CxiEndpoint::disconnectUnlocked() {
    // For CXI RDM endpoints, we don't need to reset QP state
    // Just remove peer from AV if needed and mark as disconnected
    peer_fi_addr_ = FI_ADDR_UNSPEC;
    status_.store(UNCONNECTED, std::memory_order_release);
}

const std::string CxiEndpoint::toString() const {
    return "CxiEndpoint[" + context_.nicPath() + " <-> " + peer_nic_path_ + "]";
}

bool CxiEndpoint::hasOutstandingSlice() const { return wr_depth_ > 0; }

int CxiEndpoint::doSetupConnection(const std::string &peer_addr,
...
void CxiEndpoint::associateWithMr(struct fid_mr *mr) {
    int ret = fi_mr_bind(mr, &ep_->fid, 0);
    if (ret) {
        LOG(ERROR) << "aoooo\n";
    }
    ret = fi_mr_enable(mr);
    if (ret) {
        LOG(ERROR) << "forza roma\n";
    }
}

int CxiEndpoint::submitPostSend(
    ...
    for (auto it = slice_list.begin(); it != slice_list.end();) {
...

        {
            ...
            CxiOpContext *op_ctx = new CxiOpContext();
            memset(op_ctx, 0, sizeof(CxiOpContext));
            op_ctx->slice = slice;
            ...
            std::cout << "dest_addr: " << std::hex << slice->rdma.dest_addr << " dest key: " << std::hex << slice->rdma.dest_rkey << '\n';
    ...
    timeout:
        LOG(WARNING) << "CXI submitPostSend: timed out waiting for CQ drain"
