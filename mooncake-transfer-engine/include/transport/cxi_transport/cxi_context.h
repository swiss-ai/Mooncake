...
class CxiContext {
   ...
   public:
    ...
    std::shared_ptr<CxiEndpoint> peekEndpoint(const std::string& peer_nic_path);
   ...
   private:
    ...
    std::shared_ptr<CxiCq> shared_cq_;
    ...
    std::unordered_map<std::string, std::shared_ptr<CxiEndpoint>> peer_map_;

    RWSpinlock mr_lock_;
    std::map<uint64_t, CxiMemoryRegionMeta> mr_map_;
};
